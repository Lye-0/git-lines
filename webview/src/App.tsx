...
export function App() {
  ...
  return <main className="app-shell">
    ...
    {error ? <EmptyState title={error.title} detail={error.detail} /> : graph ? <div className="content-shell"><div className="graph-content"><GraphViewport layout={graph.layout} loading={loading} onLoadMore={handleLoadMore} filter={filter} selected={selected} onSelect={(oid) => { setSelected(oid); vscode.postMessage({ type: 'select', oid }); }} />{!detail && workingTree && <WorkingTreeSummaryPanel tree={workingTree} />}</div>{detail && <DetailPanel detail={detail} title={selectedNode?.subject} refBadges={detailRefBadges} onClose={() => { setDetail(null); setSelected(undefined); }} />}</div> : <EmptyState title="Loading repository" detail="Reading Git refs, history, and working tree state…" />}
  </main>;
}
