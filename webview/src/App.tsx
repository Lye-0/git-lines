...
export function App() {
  ...
  return <main className="app-shell">
    ...
    {error ? <EmptyState title={error.title} detail={error.detail} /> : graph ? <div className="content-shell"><div className="graph-content"><GraphViewport layout={graph.layout} loading={loading} onLoadMore={handleLoadMore} filter={filter} selected={selected} selectedWorkingTree={selectedWorkingTree} selectedEvent={selectedEvent} showWorkingTreeStats={!detail && !detailEvent && !selectedWorkingNode} onSelect={(oid) => { setSelected(oid); setSelectedWorkingTree(undefined); setSelectedEvent(undefined); vscode.postMessage({ type: 'select', oid }); }} onSelectWorkingTree={(id) => { setSelected(undefined); setSelectedWorkingTree(id); setSelectedEvent(undefined); setDetail(null); setDetailEvent(undefined); }} onSelectEvent={(id) => { setSelected(undefined); setSelectedWorkingTree(undefined); setSelectedEvent(id); vscode.postMessage({ type: 'selectEvent', id }); }} /></div>{(detail || detailEvent || selectedWorkingNode) && <DetailPanel detail={detail ?? undefined} event={detailEvent} workingTree={selectedWorkingNode?.workingTree} operation={selectedWorkingNode?.operation} sourceCommits={workingSourceCommits} linkedWorktrees={detailNode?.linkedWorktrees} title={detailNode?.subject} routeName={detailRouteName} headState={detailHeadState} refBadges={detailRefBadges} onClose={() => { setDetail(null); setDetailEvent(undefined); setSelected(undefined); setSelectedWorkingTree(undefined); setSelectedEvent(undefined); }} />}</div> : <EmptyState title="Loading repository" detail="Reading repository state…" />}
  </main>;
}
