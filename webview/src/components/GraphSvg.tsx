...
export function GraphSvg({ layout, width }: { layout: GraphLayout; width: number }) {
  ...
  return <svg className="graph-svg" width={width} height={Math.max(50, layout.nodes.length * layout.rowHeight)} aria-hidden="true">
    {paths.map((edge) => { const track = byId.get(layout.edges.find((candidate) => candidate.id === edge.id)?.fromNodeId ?? '')?.trackId; return <path key={edge.id} d={edge.d} className={`edge edge-${edge.type}`} stroke={colorByTrack.get(track ?? '') ?? 'var(--graph-muted)'} opacity={opacityByTrack.get(track ?? '') ?? 1} />; })}
