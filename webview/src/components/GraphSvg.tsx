...
export function GraphSvg({ layout, width, height }: { layout: GraphLayout; width: number; height?: number }) {
  ...
  const visiblePaths = paths.filter((edge) => {
    if (edge.type !== 'history-event' || !edge.id.endsWith(':from')) return true;
    const source = byId.get(edgeById.get(edge.id)?.fromNodeId ?? '');
    const event = byId.get(edgeById.get(edge.id)?.toNodeId ?? '');
    // A same-lane source connector only duplicates the normal parent rail and
    // creates a long dashed braid when many reflog entries are present. Keep
    // it for genuine cross-lane moves; the event-to-destination edge remains.
  ...
  return <svg className="graph-svg" width={width} height={height ?? Math.max(50, layout.nodes.reduce((max, node) => Math.max(max, (node.row ?? 0) + 1), 0) * layout.rowHeight)} aria-hidden="true">
    {visiblePaths.map((edge) => { const track = byId.get(edgeById.get(edge.id)?.fromNodeId ?? '')?.trackId; return <path key={edge.id} d={edge.d} className={`edge edge-${edge.type}`} stroke={colorByTrack.get(track ?? '') ?? 'var(--graph-muted)'} opacity={opacityByTrack.get(track ?? '') ?? 1} />; })}
