...
export function GraphSvg({ layout, width }: { layout: GraphLayout; width: number }) {
  ...
  return <svg className="graph-svg" width={width} height={Math.max(50, layout.nodes.length * layout.rowHeight)} aria-hidden="true">
    {layout.edges.map((edge) => { const from = point(edge.fromNodeId); const to = point(edge.toNodeId); const track = byId.get(edge.fromNodeId)?.trackId; return <path key={edge.id} d={pathFor(from.x, from.y, to.x, to.y)} className={`edge edge-${edge.type}`} stroke={colorByTrack.get(track ?? '') ?? 'var(--graph-muted)'} opacity={opacityByTrack.get(track ?? '') ?? 1} />; })}
    {layout.nodes.map((node) => { const p = point(node.id); const track = colorByTrack.get(node.trackId ?? '') ?? 'var(--graph-muted)'; const symbol = node.kind === 'fast-forward-event' ? '◇' : node.kind === 'reflog-commit' ? '◌' : node.kind === 'history-boundary' ? '⋯' : node.kind === 'working-tree' || node.kind === 'operation' ? '○' : node.kind === 'history-event' ? '◇' : '●'; return <g key={node.id} transform={`translate(${p.x},${p.y})`} className={`node node-${node.kind}`} style={{ color: track, opacity: opacityByTrack.get(node.trackId ?? '') ?? 1 }}><text x="0" y="1" textAnchor="middle">{symbol}</text></g>; })}
  </svg>;
}
