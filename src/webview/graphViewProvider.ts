...
/** A VS Code-owned webview next to Terminal / Output. */
export class GraphViewProvider implements vscode.WebviewViewProvider, vscode.Disposable {
  ...
  public resolveWebviewView(view: vscode.WebviewView): void {
    ...
    this.session = new GraphViewSession(this.context, view.webview, this.repositoryRoot);
  ...
  public async open(repositoryRoot: string): Promise<void> {
    ...
    if (this.view && this.session?.repositoryRoot !== repositoryRoot) {
      ...
      this.session = new GraphViewSession(this.context, this.view.webview, repositoryRoot);
    }
