...
`resources/icon-v1.png` は組み込み image_gen で生成・背景調整し、ユーザーが採用した1254×1254pxのPNG。Marketplace用の拡張アイコンとしてpackage.jsonから参照する。修正時は原稿を残し、次のversionの画像へ更新する。
...
背景調整前の比較用画像は `icon-v1-original.png` に保存。design資料はVSIXへ含めない。
