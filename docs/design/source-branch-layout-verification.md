...
これは製品実装前の試作の検証記録。試作段階では製品の`src/`、通常ビルド、既存の`test`リポジトリの履歴を変更していない。

後続の製品実装では`branchLineage.ts`と`sourceBranchLayout.ts`へ型付き処理を移し、セッションの自動選択と明示指定も分離した。製品版で445テスト、重点168ケース、広域1,104ケースを再検証済み。現在の契約は`docs/technical/graph-architecture.md`、実装時の集計は`research/lineage/implementation-summary.json`を参照。元の試作比較は固定基準commitから読込むため再実行可能。

製品実装後、隔離したVS Codeプロファイルで148のStandard/Fixed切替を目視し、D0を含むmainの左列とfeatureの独立列を確認した。14の入れ子、28の複数merge、113の相互mergeも表示確認し、113はAuto未判定からmainを手動指定したFixedでも確認した。これはエディタ表示のスポット確認で、全シナリオ・全操作のGUI網羅ではない。循環へ流入する分岐名も自動基準に採用しない追加チェックは製品の単体テストで検証する。
