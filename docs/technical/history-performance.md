...
Source-branch placement resolves lineage from the protection reflogs already cached for both modes. Shared tips with unknown creation records can additionally require bounded first-parent metadata to preserve the same route across pagination. `readRouteContinuityEvidence` reads at most 64 missing commit objects, traverses at most 256 steps per candidate, reuses the object cache and one lazy batch reader, and stops on foreign creation evidence. These objects are passed only through `routeEvidenceCommits`: snapshot commits, visible counts and graph nodes are not expanded. Presentation changes reuse the session's evidence; fresh snapshot reads invalidate it. No complete-history traversal is added. Explicit primary settings are distinguished from automatically selected refs at the session boundary. See `docs/design/source-branch-layout-verification.md` for the original validation scope.
