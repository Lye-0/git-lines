...
グラフのSVGレイヤーにはレーン用の左余白を確保し、HTMLのcommit行はその右側から開始する。Ref Eventは接続先commitのレーンを変えず、commit位置からイベントglyphへ横方向に伸びる単一の破線annotationとして描画する。イベントの`from`側をcommit DAGの線として描画しない。Working Tree / operationは点線、commit parentは実線で表示する。通常commitの行はcommit種別を繰り返さず、subjectとshort hash・author・相対時刻を主表示にする。凡例はtoolbarのpopoverから参照できる。
...
- `tests/unit/layout.test.ts` — row一意性、timestamp inversion、primary lane、distance-based feature lane、Ref Event annotation
- `tests/unit/history-events.test.ts` — FF/reset/amend/rebaseの保守的分類
- `tests/unit/graph-builder.test.ts` — ref dedup、tag、常時Working Tree、2/3-parent edge、Ref Event annotation
