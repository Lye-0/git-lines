...
- `fast-forward-event` / `history-event` — Amend以外の、reflogから確認できるref移動の時系列行。commitとは別モデルで、`anchorCommitId`は移動先、`eventBoundaryCommitId`は操作後の新しい履歴区間の直下、`eventStartCommitId`はその区間の直上にある注釈接続元を参照する。Amendは`HistoryRelation`としてDAGのrow外へ分離する
...
実在のparent関係は`GraphEdge.type = "parent"`で表し、`GitCommit.parentOids`の各要素から1本ずつ生成する。laneや表示都合でparent edgeを削除・統合しない。Working Treeと未完了operationは同じ`working-tree` nodeにまとめ、HEADへは`working-tree` edge、operationのsource commitへは`operation` edgeをWorking Treeから直接張る。通常のref移動は`history-event`で表し、Amendだけは`HistoryRelation`としてDAGのnode / row / lane / edgeとは別に表す。Ref Eventには、移動先を保持する`anchorCommitId`、行配置の意味的な下側境界を保持する`eventBoundaryCommitId`、注釈線の上側接続元を保持する`eventStartCommitId`を使う。annotation edgeには`annotation: "ref-event"`を付け、Cherry-pick / Revertは生成commitのparent直上、複数commit Rebaseは再構築区間の最下端（onto commitの直上）、Resetは移動先commitの直上へevent rowを挿入する。Amendはreflogの旧OID→新OIDを`HistoryRelation`へ保持し、両端が現在表示可能なときだけ短い矢印付きoverlayを描画する。旧commitのPREVIOUS / Historical判定はこのrelationとは独立して行い、Amendのためにevent rowや偽のparent edgeは作らない。確実なbranch rename reflogがある場合は、commit DAGを変更せず同じroute上のWorking TreeとHEAD commitの間にrename event rowを挿入し、元のWorking Tree edgeを同じ曲線上でevent位置の前後へ分割する。事実モデルのWorking Tree edgeとrename annotation edgeは保持するが、描画では前者の2本のdotted pathだけを出してannotationの縦stubを重ねない。Rebaseの長い区間では最古の再構築commitからglyphへ補助接続する。patchが似ているだけのsquash/cherry-pick/rebase前後をparent edgeへ変換しない。
...
`MERGE_HEAD`、`REBASE_HEAD`、`CHERRY_PICK_HEAD`、`REVERT_HEAD`などが現在存在する場合のみoperation stateをWorking Tree nodeへ付与する。operation edgeは点線で、commit parent edgeとは混同しない。objectがGC済みのreflog OIDは表示しない。Reset / Amend / 完了Rebaseの証拠はreflog subjectと実在OIDから保守的に解決し、Resetは旧経路に表示可能な実commitがある場合、Amendはfrom/to両OIDが取得できる場合に`HistoryRelation`を生成する。Amendの旧commitをPREVIOUSへ分類するかどうかはrelationの有無とは独立する。Rebaseは対象local branchの`rebase (finish)` reflogに実在するfrom/to OIDがある場合に限って生成する。完了Cherry-pick / Revertはlocal branch reflogの明示的な操作subjectと実在するfrom/to commitがある場合だけ生成する。Rebase中のHEAD start/continue/finish(returning) entryは内部移動として除外し、branchのfinish entryへ集約する。Merge完了は実際のmulti-parent commit自体が操作を表すためHistory Eventへ昇格しない。`Branch: renamed refs/heads/<old> to refs/heads/<new>`というGitの明示的なrename reflogは、同じOIDを維持するref名操作としてHEAD / 新branch refの重複を1件へまとめ、同じlive routeのWorking Treeと対象commitの間へ配置する。旧経路が存在しない通常のreflog移動はEventへ昇格しない。生成されたEventを選択すると、operation、branch / ref、old / new hash、timestamp、raw reflog messageをDetailで確認できる。Reset / Rebase eventのtrackはref名だけで固定せず、現在の`toOid`が割り当てられたrouteを基準に解決する。後続のref移動でdestinationがPREVIOUSになった場合は同じHistorical Routeへ移し、live destinationの場合だけ現在のref laneへ置く。
...
3. `buildGraphFacts`がcommit dedup、ref association、Working Tree（必要ならoperation付き）/event nodeと、Amendのoverlay relationを作る。
...
Reset、Branch move、force update、generic ref moveのうち、旧経路をHistoricalとして表示する必要がないものは`refOnly` eventとして扱う。これらはcommit-producing eventのsemantic boundaryへ挿入せず、Working Treeの直後から最初のDAG nodeまでをreflogの実時系列で埋める専用rowへ配置する。model上の`annotation: "ref-event"`はref操作の事実とdetail用の接続先を保持するが、描画ではcommitへ戻るstubを出さず、実DAGのparent関係とWorking Tree edgeを変更しない。後方ResetについてはGitのancestor関係と`newOid..oldOid`の実commit集合が確認できた場合だけ、除外範囲と件数をeventへ保持する。reflogからmodeを確定できない場合は表示せず推測しない。Reflog OFFではref-only eventとrowを除き、通常のDAGへ戻す。

### Operation Overlay presentation

Amend overlayのcurve、arrowhead、diamond、labelは`--operation-overlay-accent`を共有し、通常のbranch DAG色から独立した専用accentで描画する。overlayの色はrelation表示だけに適用し、DAGのnode / edge色や履歴判定は変更しない。

Amend relationが表示対象になる場合は、`OperationAnnotationRow`をlayout上の仮想timeline rowとして1行だけ確保する。このrowはmessage columnに操作詳細を表示し、graph側の`◇ Amend`は既存relation curve上に置く。rowはcommit node、DAG edge、lane、parent relationには参加せず、Reflog OFFまたはsource/target未ロード時はrelationとともに消える。
...
- `tests/unit/graph-builder.test.ts` — ref dedup、tag、常時Working Tree、remote-ahead時の実HEAD接続、2/3-parent edge、destination/boundary/targetRef付きRef Event
- `tests/unit/ref-display.test.ts` / `tests/unit/graph-metrics.test.ts` — 多数refのbadge保持とchanges column用横幅確保
- `tests/integration/git-client.test.ts` — 実Git CLIのin-progress operation、branch rename、完了Cherry-pick/Revert、single/multi-commit Rebase、実MergeでHistory Eventを作らないこと、long feature merge、未commitbranch、first commit、後続ResetでHistoricalへ移る旧Reset event
