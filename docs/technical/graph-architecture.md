...
- `reflog-commit` — 現在のuser-facing refから到達できず、reflogから確認でき、objectが残っているcommit。Reset/Amend/完了Rebaseの旧経路だけ`previousRoute`を持ち、行表示では`PREVIOUS`になる
...
- `tests/unit/history-events.test.ts` — FF/reset/amend/rebase/cherry-pick/revertの保守的分類、完了Rebaseの内部HEAD entry集約、semantic boundary、`old..new`件数、操作名
- `tests/unit/event-presentation.test.ts` — FF/reset/amend/rebase/cherry-pick/revertラベル、幅別compact表示、content column配置、tooltip情報
...
- `tests/integration/git-client.test.ts` — 実Git CLIのin-progress operation、完了Cherry-pick/Revert、single/multi-commit Rebase、実MergeでHistory Eventを作らないこと、long feature merge、未commitbranch、first commit、後続ResetでHistoricalへ移る旧Reset event
