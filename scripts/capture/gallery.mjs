...
export async function gallery(run) {
  ...
  const settings = JSON.parse(await fs.readFile(path.join(run,'profile/User/settings.json'),'utf8'));
  const reflogLabel = settings['branchGraph.showReflog'] === false ? 'Off' : 'On';
  const captures = [];
