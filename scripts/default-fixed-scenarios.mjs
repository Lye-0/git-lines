  ...
  for (const scenario of scenarios) {
    ...
    for (const showReflog of [false, true]) {
      ...
      for (const [host, rowHeight, laneWidth] of [['sidebar', 28, 22], ['compact', 30, 34], ['comfortable', 38, 34]]) {
        ...
        if (showReflog && ['ff', 'deleted', 'continued'].includes(scenario.kind)) for (const layout of [standard, fixed]) {
          const ff = layout.nodes.find((n) => n.kind === 'fast-forward-event');
          assert(ff && Number.isFinite(ff.visualX), `${id}: FF diamond on existing curve`);
