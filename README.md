...
レーン配置は **Standard**（従来の配置・初期値）と **Default Fixed**（default列を左端に固定） から選べます。固定モードは他ブランチの列を保護し、作成元がreflogで確認できるfeatureのコミットはfast-forward後もdefaultの列へ移しません。Reflog OFFでも、この保護のために記録を内部利用します。過去の作成ブランチを完全に復元する機能ではなく、情報が不足する区間は従来の識別を維持します。
